...
#####
#
# Updated to blender 2.8 by HikariTW
#
bl_info = {
    "name": "OFF format",
    "description": "Import-Export OFF, Import/export simple OFF mesh.",
    "author": "Alex Tsui, Mateusz Kłoczko, hikariztw",
    "version": (0, 4, 0),
    "blender": (2, 80, 0),
    "location": "File > Import-Export",
    "warning": "Testing script for medical surgery project usage only",
...
class ExportOFF(bpy.types.Operator, ExportHelper):
    ...
    use_world_matrix: BoolProperty(
        name="World Location",
        description="Export using world matrix instead of local coordinates",
        default=False,
    )
    use_modifiers: BoolProperty(
        name="Apply Modifier",
        description="Apply the modifier for exported mesh",
        default=True,
    )
...
def save(
    ...
    if len(context.selected_objects) < 1:
        operator.report({"ERROR"}, "No Object selected")
        return {"CANCELLED"}
    obj = context.selected_objects[0]

    mesh = obj.to_mesh(
        preserve_all_data_layers=use_modifiers,
    ...
    fp.close()
    operator.report({"INFO"}, "Export as:{}".format(filepath))
    return {'FINISHED'}


# Register part
__classes__ = (ImportOFF, ExportOFF)
